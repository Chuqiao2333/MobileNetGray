...
classes = []
for i in range(1000):
    classes.append(class_info_dict[str(i)][0])
...
parser.add_argument('-number_of_classes', default = 8, type=int)
...
parser.add_argument('-class_list', default=classes, nargs='*')
...
if not os.path.isdir(args.data_root):
    logging.error('folder {args.data_root} does not exist! please provide existing folder in -data_root arg!')
    exit()
...
if args.use_class_list == True:
   for item in args.class_list:
       ...
       if item not in class_info_dict:
           logging.error('Class {item} not found in ImageNete')
           exit()
...
def get_image(img_url):
    ...
    def finish(status):
        ...
        if status == 'success':
            ...
            logging.error('No such status {status}!!')
            exit()
        return
    ...
    logging.debug('image size {len(img_resp.content)}')
